<!-- ===================================== -->
<!--   GitHub Profile README | Fog         -->
<!--   Premium · No Stats · Story Edition  -->
<!-- ===================================== -->

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:2C5364,50:203A43,100:0F2027&height=180&section=header&text=Fog%20%7C%20Product%20Engineer&fontSize=36&fontColor=ffffff&animation=fadeIn" />
</p>

<p align="center">
  <a href="./README.md"><b>English</b></a> ·
  <a href="./README.ko.md">한국어</a>
</p>

<h2 align="center">Android Developer · Product Engineer</h2>

<p align="center">
  <b>15+ years of Android Native Development</b><br/>
  Clean Architecture · Pragmatic Engineering · Products that ship 🚀
</p>

<p align="center">
  <a href="https://github.com/dreamboy1339">
    <img src="https://img.shields.io/badge/GitHub-dreamboy1339-181717?logo=github"/>
  </a>
</p>

---

## 👋 About Me

- 📱 **Android Native Developer (15+ years)**
  - Java → Kotlin → Jetpack Compose까지 전 세대 실전 경험
- 🧩 **Clean Architecture / MVVM / MVI** 중심의 구조 설계
- 🎨 UI·UX와 코드 품질을 함께 고려하는 **Product-minded Engineer**
- ⚙️ **Embedded (STM32)**, **Rust**  
  → *아래 레이어를 이해하는 개발자*로 확장 중
- ✍️ 배운 것은 정리하고 공유하는 스타일

> “코드는 수단이고,  
> **유지 가능한 제품을 만드는 것이 목표다**.”

---

## 🧭 Fog’s Career Story

나는 커리어 초반부터  
**“기능을 빨리 만드는 개발자”**보다는  
**“시간이 지나도 망가지지 않는 구조를 만드는 개발자”**에 더 끌렸다.

Android 초창기 시절부터 여러 서비스의  
- 커지는 코드베이스  
- 반복되는 UI 수정  
- 유지보수 비용이 눈덩이처럼 불어나는 상황  
을 직접 겪으며 깨달은 게 있다.

> **문제는 코드가 아니라, 구조였다.**

그 이후부터 나는  
- 왜 이 레이어가 필요한지  
- 이 의존성은 어디까지 허용해야 하는지  
- 이 기능은 1년 뒤에도 버틸 수 있는지  
를 먼저 고민하는 개발자가 되었다.

최근에는 모바일을 넘어  
**Embedded 시스템과 Rust**를 공부하며  
“위에서 잘 만드는 것”뿐 아니라  
**“아래에서 어떻게 돌아가는지 이해하는 엔지니어”**로 확장 중이다.

지금도 내 기준은 단순하다.

> ✨ *지금도, 1년 뒤에도,  
> 그리고 다른 사람이 이어받아도  
> 이해할 수 있는 코드를 만드는 것.*

---

## 🛠 Tech Stack

### 📱 Mobile

<p>
  <img src="https://img.shields.io/badge/Kotlin-7F52FF?logo=kotlin&logoColor=white"/>
  <img src="https://img.shields.io/badge/Android-3DDC84?logo=android&logoColor=white"/>
  <img src="https://img.shields.io/badge/Jetpack%20Compose-4285F4?logo=android&logoColor=white"/>
</p>

- Android SDK (API 21+)
- Jetpack Compose / View System
- Coroutines / Flow

---

### 🧱 Architecture & Engineering

- Clean Architecture
- MVVM / MVI
- Multi-module Gradle (Kotlin DSL)
- Testable & maintainable codebase
- Dependency Injection (DI-free 설계도 선호)

---

### ⚙️ Also Exploring

- 🦀 Rust
- 🔌 Embedded systems (STM32)
- 📐 System design & performance optimization

---

## 🎯 What I’m Focused On

- ✨ **Jetpack Compose 기반 UI 아키텍처 고도화**
- 🧪 재사용 가능한 Android core / network / domain 모듈 설계
- 🧠 모바일 개발자의 시야 확장을 위한 저수준 학습
- ✍️ 실무 기반 기술 정리 & 글쓰기

---

## 🧠 Engineering Philosophy

- 🧱 Architecture exists to **support change**, not to show off
- 🧪 Testability is a **design choice**, not an afterthought
- 🎨 UI is part of the system, not decoration
- 📦 Reusability matters only when it solves real problems
- 🚀 Shipping matters — but never at the cost of maintainability

> “Good engineers write code.  
> **Great engineers design for tomorrow.**”

---

## 📦 Selected Work

> 숫자보다 **어떻게 설계하는지**를 더 중요하게 생각합니다.

- 🧩 **Android Architecture Templates**
  - Clean Architecture 기반 Android 프로젝트 구조
  - Multi-module · Compose-friendly · Testable
  - (Private / WIP)

- 🧪 **Reusable Android Core Modules**
  - Network / Domain / UI Layer 분리
  - 실서비스 기준으로 다듬는 중

- ⚙️ **Embedded & Low-level Experiments**
  - STM32, Rust 학습 및 실험 코드
  - 모바일 개발자의 시스템 이해 확장 목적

---
<!--
## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=dreamboy1339&show_icons=true&hide_title=true&theme=dark" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=dreamboy1339&layout=compact" />
</p>

---
-->
## 📬 Contact

- GitHub: [@dreamboy1339](https://github.com/dreamboy1339)

---

<p align="center">
  <i>
  숫자보다 구조를, 속도보다 지속 가능성을.<br/>
  오래 가는 제품을 만드는 엔지니어를 지향합니다.
  </i>
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:203A43,100:2C5364&height=120&section=footer"/>
</p>
